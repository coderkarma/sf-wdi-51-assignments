...
console.log(`${greeting} I am ${name}, I'm ${age} years old and live ${address}.`);
...
console.log(`${greeting2} I am ${name2}, I'm ${age2} years old and live ${address2}.`);
...
*/
//   My pesuedo code for finding largest number
//   1. Declare  variable largest to store the value
//  loop through the array of numbers

//  compare the current number to the next number 
//  assign the  variable largest number to the current number 
//  check to see if the next number is greater then the current number, if it is then we would get the largest number, if not then we move to next one
// return the largest numbr 
...
function largestNumber(numbers) {
	...
	for (let i = 0; i < numbers.length; i++) {
		// declare the currentNum
		let currentNum = numbers[i];
		// check to see if the current number is greater then next number
		if (currentNum > largestNum || largestNum === 0) {

			largestNum = currentNum;
		}
	}
	return `The highest number is ${largestNum}`;
}
console.log(largestNumber([20, 3, 5, 22, 13, 30])) 


// let largestNum = 0;
//   for (let i = 0; i <numbers.length; i++){
// 	  let currenNumber = numbers[i];
	  
// 	  if( currenNumber > largestNum){
// 		  largestNum = currenNumber;
// 	  }
//   }
